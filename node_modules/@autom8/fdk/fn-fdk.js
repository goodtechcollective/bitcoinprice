...
/*
  "repository": {
    "type": "git",
    "url": "https://github.com/fnproject/fdk-node.git"
  },
 */
 ...
 */
fnFdkFunctions.handle = function (fnfunction, options) {
 ...
 */
fnFdkFunctions.streamResult = function (stream) {
 ...
 * @returns {RawResult}
 */
fnFdkFunctions.rawResult = function (res) {
  ...
  let functionHandler = (req, resp) => {
    ...
    req.on('data', chunk => {
      ...
      new Promise(function (resolve, reject) {
        ...
        .then((result) => {
          // transform the output if there is a transform function to transform
          if (transformer[body._transformer]) {
            return transformer[body._transformer](result)
          }
          return result
        })
        .then((result) => {
...
/**
 * This is a handler that outputs a Proxy. A Proxy allows calling arbitrary names off of an object (in this case the export)
 * When a function name is called on the Proxy if it is an essentila fdkFunction it is treated like normal. Otherwise it
 * is assumed to be a data transformer that will transform the output of the fn Function if the input value
 * {"_transformer": name} matches it.
 *
 * @return {Proxy}
 */
const fdkHandler = () => {
  ...
  const callFunction = (...args) => {
    const functionName = callFunction.call.join('.')

    // reset call to blank
    callFunction.call = []

    const fdkFunction = ['handle', 'streamResult', 'rawResult']
    // if it's an fdk function do that with inputs, if it's a transformer then run the transformer
    if (fdkFunction.includes(functionName)) {
      return fnFdkFunctions[functionName](...args)
    }
    // othrwise set the transform map with a given function type
    transformer[functionName] = args[0]
  }
